...
public class Gestore {
	...
	public int richiesta1() throws InterruptedException, IllegalUseOfConditionException {		
		...
		printUtility();
		try {
			if (disp == 0) // Non serve il while perchŽ se qualcuno l'ha svegliato gli ha passato la condizione
						   // quindi sicuramente c' una risorsa disponibile per lui.
				singola.await();	
			disp --;
			while(!libera[i])
				i++;
	...
	public Coppia richiesta2() throws InterruptedException, IllegalUseOfConditionException {		
		...
		printUtility();
		try {
			if (disp < 2) // Non serve il while perchŽ se qualcuno l'ha svegliato gli ha passato la condizione
						  // quindi sicuramente c' una coppia di risorse disponibile per lui.
				doppia.await();	
			disp = disp - 2;
			while(!libera[i])
				i++;
